...
__all__ = ["AsyncUserEventsService", "UserEvent"]


class AsyncUserEventsService(AsyncService):
    ...
    async def stream(self, *, types: Collection[str] | None = None) -> AsyncIterator[UserEvent]:
        ...
        parser = SseParser()
        async with self._transport.stream(ops.user_events()) as response:
