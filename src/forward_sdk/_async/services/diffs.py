...
class AsyncSnapshotDiffsService(AsyncService):
    ...
    async def subnet_connectivity(self, before: str, after: str) -> SubnetConnectivityDiff:
        ...
        does that for you.
        """
        payload = await self._send_json(ops.subnet_connectivity(before=before, after=after))
        return SubnetConnectivityDiff.model_validate(payload or {})

    async def wait_for_subnet_connectivity(
        self,
        before: str,
        after: str,
        *,
        ...
        while True:
            result = await self.subnet_connectivity(before, after)
    ...
    async def vulnerability_counts(self, before: str, after: str) -> VulnerabilityDiffCounts:
        ...
        """
        payload = await self._send_json(ops.vulnerability_counts(before=before, after=after))
        return VulnerabilityDiffCounts.model_validate(payload or {})

    async def routing_loops(self, before: str, after: str) -> RoutingLoopDiffCount:
        ...
        """
        payload = await self._send_json(ops.routing_loops(before=before, after=after))
        return RoutingLoopDiffCount.model_validate(payload or {})

    async def routing_loops_bidirectional(self, before: str, after: str) -> BidirectionalDiffCount:
        """Loops the change introduces and loops it resolves, separately.

        Needs the PREDICT_DIFF_INSIGHT_ENHANCEMENTS org property.
        """
        payload = await self._send_json(ops.routing_loops_bidirectional(before=before, after=after))
        return BidirectionalDiffCount.model_validate(payload or {})

    async def vulnerabilities_bidirectional(
        ...
        payload = await self._send_json(
            ops.vulnerabilities_bidirectional(before=before, after=after)
        )
        return BidirectionalVulnerabilityDiff.model_validate(payload or {})

    async def file_summary(self, before: str, after: str) -> FileDiffSummary:
        """How many devices and locations the change touches, directly and indirectly."""
        payload = await self._send_json(ops.file_summary(before=before, after=after))
        return FileDiffSummary.model_validate(payload or {})

    async def count(self, area: str, before: str, after: str) -> DiffCount:
        ...
        payload = await self._send_json(builder(before=before, after=after))
        return DiffCount.model_validate(payload or {})

    async def counts(self, before: str, after: str) -> dict[str, DiffCount]:
        """Every per-area count at once, keyed by area.

        Eight requests. The one-line answer to "what did this change touch".
        """
        return {area: await self.count(area, before, after) for area in ops.COUNT_BUILDERS}
