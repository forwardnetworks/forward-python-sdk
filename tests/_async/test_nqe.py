...
class TestResults:
    ...
    async def test_rows_are_passed_through_unaltered(self, recorder: Recorder) -> None:
    ...
    async def test_one_page_and_every_row_agree_on_shape(self, recorder: Recorder) -> None:
        ...
        async with make_client(recorder) as client:
            execution = await client.nqe.execute("q")
            paged = (await execution.result_page(offset=0, limit=1)).items
            iterated = [r async for r in execution.rows(page_size=10)]
...
class TestDiff:
    ...
    async def test_diff_page_fetches_one_page_only(self, recorder: Recorder) -> None:
        ...
        async with make_client(recorder) as client:
            result = await client.nqe.diff_page(
    ...
    async def test_diff_page_honours_offset_and_limit(self, recorder: Recorder) -> None:
        recorder.add(
            "POST",
            "/api/nqe-diffs/100/101",
    ...
    async def test_diff_page_rejects_inline_source(self, recorder: Recorder) -> None:
        ...
        async with make_client(recorder) as client:
            with pytest.raises(ForwardConfigurationError, match="committed query"):
                await client.nqe.diff_page("foreach d in x select {}", before="1", after="2")
