...
async def test_security_zones_is_a_device_to_zone_names_map(recorder: Recorder) -> None:
    recorder.add(
        "GET", "/api/networks/101/security-zones", json_response({"fw01": ["dmz", "inside"]})
    )
    async with make_client(recorder) as client:
        zones = await client.security_zones.get_security_zones()
...
async def test_blast_radius_sends_no_dispatch_query(recorder: Recorder) -> None:
    recorder.add(
        "POST",
    ...
    async with make_client(recorder) as client:
        await client.blast_radius.get_blast_radius(body=BLAST_BODY)

    assert "type" not in recorder.query_for()


async def test_host_centric_blast_radius_dispatches_on_the_same_path(recorder: Recorder) -> None:
    ...
    async with make_client(recorder) as client:
        result = await client.blast_radius.get_host_centric_blast_radius(body=BLAST_BODY)
...
async def test_resource_pool_is_sent_with_its_discriminator(recorder: Recorder) -> None:
    ...
    async with make_client(recorder) as client:
        await client.resource_pools.analyze_resource_pool(body=pool)

    assert recorder.query_for()["action"] == ["analyze"]
    assert recorder.body_for() == pool


async def test_host_centric_blast_radius_report_streams_the_workbook(recorder: Recorder) -> None:
    recorder.add(
        "POST",
    ...
    async with make_client(recorder) as client:
        chunks = [
            c async for c in client.blast_radius.get_host_centric_blast_radius_report(body=body)
        ]
...
async def test_security_matrix_filter_crud(recorder: Recorder) -> None:
    recorder.add(
        "POST",
    ...
    async with make_client(recorder) as client:
        created = await client.security_matrix_filters.add_security_matrix_filter(
            body={
                "name": "f1",
        ...
        await client.security_matrix_filters.delete_security_matrix_filter(filter_id="1")

    assert recorder.requests[-1].url.path == "/api/networks/101/securityMatrixFilters/1"


async def test_exposed_hosts_are_read_by_snapshot_not_network(recorder: Recorder) -> None:
    ...
    async with make_client(recorder) as client:
        result = await client.internet_exposure.get_internet_exposed_hosts(snapshot_id="9")
