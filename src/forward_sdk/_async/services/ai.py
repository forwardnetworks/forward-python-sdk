...
__all__ = ["AsyncAiConversation", "AsyncAiService"]
...
class AsyncAiConversation:
    ...
    Obtained from :meth:`AsyncAiService.start` or :meth:`AsyncAiService.get`.
    """

    def __init__(self, service: AsyncAiService, chat: AiChat) -> None:
    ...
    async def refresh(self) -> AiChat:
        """Fetch the chat's current state."""
        payload = await self._service._send_json(ops.get_chat(chat_id=self.id))
    ...
    async def messages(self, *, since: str | None = None) -> list[AiMessage]:
        """The questions asked and the answers given, oldest first."""
        payload = await self._service._send_json(ops.list_messages(chat_id=self.id, since=since))
    ...
    async def rename(self, name: str) -> AiChat:
        payload = await self._service._send_json(ops.rename_chat(chat_id=self.id, name=name))
        self._chat = AiChat.model_validate(payload or {})
        return self._chat

    async def delete(self) -> None:
        await self._service._send_json(ops.delete_chat(chat_id=self.id))

    async def transcript(self, *, format: str = "MARKDOWN") -> str:
        """Render the whole conversation as Markdown, HTML or JSON."""
        response = await self._service._transport.send(
            ops.transcript(chat_id=self.id, format=format)
        )
        return response.text


class AsyncAiService(AsyncService):
    ...
    async def list(self) -> list[AsyncAiConversation]:
        """The calling user's chats, most recent first as Forward returns them."""
        payload = await self._send_json(ops.list_chats())
        rows = (payload or {}).get("chats") or []
        return [AsyncAiConversation(self, AiChat.model_validate(row)) for row in rows]

    async def get(self, chat_id: str) -> AsyncAiConversation:
        payload = await self._send_json(ops.get_chat(chat_id=chat_id))
