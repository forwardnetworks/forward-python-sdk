...
class AsyncNqeExecution:
    ...
    async def result_key(self) -> str | None:
...
class AsyncNqeService(AsyncService):
    ...
    async def diff_page(
        ...
        *,
        ...
        ref = await self._diffable(query)
        self._transport.counters.increment("nqe_diff_calls")
        data = await self._diff_page(ref, before=before, after=after, offset=offset, limit=limit)
        return NqeDiffResult.model_validate(data)

    async def _diffable(self, query: QueryRef | str) -> QueryRef:
