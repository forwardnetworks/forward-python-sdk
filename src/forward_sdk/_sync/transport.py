# Generated from src/forward_sdk/_async/transport.py by scripts/unasync.py -- do not edit.
...
class Transport:
    ...
    def _default_throttle(config: ClientConfig) -> Throttle | None:
        if config.rate_limit_rpm:
            return RateLimiter(config.rate_limit_rpm)
        return None
    ...
    def send(self, spec: RequestSpec) -> httpx.Response:
        ...
        for attempt in range(policy.max_attempts):
            ...
            if response.status_code in RETRYABLE_STATUS and spec.idempotent and not final:
                ...
                self._sleep_before_retry(spec, attempt, delay, f"HTTP {response.status_code}")
                continue
