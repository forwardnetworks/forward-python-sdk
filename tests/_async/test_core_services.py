...
class TestNetworks:
    ...
    async def test_create_sends_the_name_as_a_query_parameter(self, recorder: Recorder) -> None:
        """Forward's create endpoint takes a name in the query and no body."""
        recorder.add(
            "POST", "/api/networks", json_response({"id": "5", "name": "New", "orgId": "7"})
        )
        async with make_client(recorder) as client:
            network = await client.networks.create("New")

        assert network.id == "5"
        assert recorder.query_for()["name"] == ["New"]
        assert recorder.body_for() is None

    async def test_create_applies_a_note_as_a_follow_up_update(self, recorder: Recorder) -> None:
...
class TestUpsertClassicDevices:
    async def test_upserts_then_reads_the_same_names_back(self, recorder: Recorder) -> None:
        ...
        async with make_client(recorder) as client:
            devices = await client.devices.upsert_classic([{"name": "sw1", "host": "10.0.0.1"}])
...
class TestPredictedSnapshots:
    ...
    async def test_a_prediction_is_not_the_latest_processed(self, recorder: Recorder) -> None:
        recorder.add(
            "GET",
            SNAPSHOTS,
            json_response(
    ...
    async def test_a_reprocessed_snapshot_still_counts(self, recorder: Recorder) -> None:
    ...
    async def test_predictions_can_be_asked_for(self, recorder: Recorder) -> None:
        recorder.add(
            "GET",
            SNAPSHOTS,
            json_response(
        ...
        async with make_client(recorder) as client:
            latest = await client.snapshots.latest_processed(include_predicted=True)

        assert latest is not None and latest.id == "predicted"

    async def test_a_snapshot_with_no_trigger_is_kept(self, recorder: Recorder) -> None:
    ...
    async def test_list_can_exclude_triggers(self, recorder: Recorder) -> None:
        ...
        async with make_client(recorder) as client:
            real = await client.snapshots.list(limit=1, exclude_triggers=["PREDICT"])

        assert [s.id for s in real] == ["r1"]
        assert "limit" not in recorder.query_for()

    async def test_the_two_answers_are_cached_apart(self, recorder: Recorder) -> None:
        for _ in range(2):
            recorder.add(
                "GET",
                SNAPSHOTS,
        ...
        async with make_client(recorder, snapshot_cache_ttl="lifetime") as client:
            without = await client.snapshots.latest_processed()
            with_predicted = await client.snapshots.latest_processed(include_predicted=True)
...
class TestUnparseableResponses:
    async def test_a_bad_payload_raises_a_catchable_error(self, recorder: Recorder) -> None:
...
class TestSnapshotResolutionCache:
    ...
    async def test_off_by_default(self, recorder: Recorder) -> None:
        recorder.add("GET", SNAPSHOTS, json_response({"snapshots": [snapshot("9")]}))
        recorder.add("GET", SNAPSHOTS, json_response({"snapshots": [snapshot("9")]}))
        async with make_client(recorder) as client:
    ...
    async def test_reuses_a_resolution_within_the_ttl(self, recorder: Recorder) -> None:
        recorder.add("GET", SNAPSHOTS, json_response({"snapshots": [snapshot("9")]}))
        async with make_client(recorder, snapshot_cache_ttl=300) as client:
    ...
    async def test_a_network_with_no_snapshot_is_a_real_answer(self, recorder: Recorder) -> None:
    ...
    async def test_different_networks_do_not_share_an_answer(self, recorder: Recorder) -> None:
        recorder.add("GET", SNAPSHOTS, json_response({"snapshots": [snapshot("9")]}))
        recorder.add(
            "GET",
        ...
        async with make_client(recorder, snapshot_cache_ttl=300) as client:
            first = await client.snapshots.latest_processed()
            second = await client.snapshots.latest_processed("202")

        assert first is not None and first.id == "9"
        assert second is not None and second.id == "7"

    async def test_a_different_scope_is_a_different_question(self, recorder: Recorder) -> None:
        ...
        async with make_client(recorder, snapshot_cache_ttl=300) as client:
            ...
            await client.snapshots.latest_collected_id(include_tags=["edge"])

        assert recorder.count("POST", "/api/nqe") == 2

    async def test_expires_after_the_ttl(
        ...
        recorder.add("GET", SNAPSHOTS, json_response({"snapshots": [snapshot("9")]}))
        recorder.add("GET", SNAPSHOTS, json_response({"snapshots": [snapshot("9")]}))
        async with make_client(recorder, snapshot_cache_ttl=60) as client:
            await client.snapshots.latest_processed()
            now[0] += 61
            await client.snapshots.latest_processed()
    ...
    async def test_uploading_invalidates_it(self, recorder: Recorder, tmp_path: Path) -> None:
        ...
        async with make_client(recorder, snapshot_cache_ttl=300) as client:
            before = await client.snapshots.latest_processed()
...
class TestDeviceTags:
    async def test_list_tags_unwraps_forwards_envelope(self, recorder: Recorder) -> None:
    ...
    async def test_list_with_devices_uses_the_dispatch_parameter(self, recorder: Recorder) -> None:
        ...
        async with make_client(recorder) as client:
            tags = await client.device_tags.list(with_devices=True)

        assert recorder.query_for()["with"] == ["devices"]
        assert tags[0]["devices"] == ["sw1"]

    async def test_add_tag_to_devices_sends_a_device_set(self, recorder: Recorder) -> None:
    ...
    async def test_remove_tag_from_devices_sends_a_device_set(self, recorder: Recorder) -> None:
        recorder.add("POST", "/api/networks/101/device-tags/core", json_response({}))
        async with make_client(recorder) as client:
