...
class TestReading:
    ...
    def test_dry_run_drops_unchanged_paths_like_commit_does(self, recorder: Recorder) -> None:
        ...
        with make_client(recorder) as client:
            report = client.nqe.repo.dry_run(["/A/q1", "/A/q2"], snapshot_id="663")

        assert report == {}
        assert recorder.count("POST", COMMITS) == 1

    def test_dry_run_retries_with_the_changed_subset(self, recorder: Recorder) -> None:
        recorder.add(
            "POST",
            COMMITS,
            error_response(
...
class TestPublishing:
    ...
    def test_a_clean_dry_run_commits(self, recorder: Recorder) -> None:
        ...
        with make_client(recorder) as client:
            report = client.nqe.repo.publish({"/A": "clean"}, title="t", dry_run_snapshot_id="9")

        assert report.committed_paths == ("/A",)
        assert report.warnings == ()
        assert recorder.count("POST", COMMITS) == 2

    def test_warnings_do_not_block_but_are_reported(self, recorder: Recorder) -> None:
        ...
        recorder.add("GET", HEAD, json_response({"id": "a" * 40}))
        with make_client(recorder) as client:
            report = client.nqe.repo.publish({"/A": "fine"}, title="t", dry_run_snapshot_id="9")
